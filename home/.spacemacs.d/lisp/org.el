...
(defun moritzs/org-export-output-file-name-modified (orig-fun extension &optional subtreep pub-dir)
  (let ((buffer-dir (file-name-directory (or (buffer-file-name) ""))))
    (unless pub-dir
      (setq pub-dir
            (if (string-match "\\(.*/wiki/\\)" buffer-dir)
                (expand-file-name "export" (match-string 1 buffer-dir))
              "exported-org-files")))
    (apply orig-fun extension subtreep pub-dir nil)))

(advice-add 'org-export-output-file-name :around #'moritzs/org-export-output-file-name-modified)
