...
function vi_mode_prompt_color() {
  echo "${${KEYMAP/vicmd/$PR_MAGENTA}/(main|viins)/$PR_CYAN}"
}
...
$PR_YELLOW%D{%H:%M:%S}\
$PR_LIGHT_BLUE%{$reset_color%}`git_prompt_info``git_prompt_status`$PR_BLUE)`vi_mode_prompt_color`$PR_HBAR\
$PR_HBAR\
>$PR_NO_COLOUR '
